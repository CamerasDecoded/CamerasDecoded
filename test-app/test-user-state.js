// test-user-state.js – Global user state management
// Loads on every page, detects auth status, fetches user data from Firestore
// Exposes window.USER with all necessary info

console.log('✅ test-user-state.js loading...');

// Initialize window.USER object with common fields
window.USER = {
  isLoggedIn: false,
  uid: null,
  name: null,
  ...
  try {
    ...
    if (doc.exists) {
      ...
      window.USER = {
        ...
        instructorTier: data.instructorTier || null,
...
console.log('✅ test-user-state.js loaded');